...
const BORDER = "#E5E7EB";
const LOGO_PATH = fs_1.default.existsSync(path_1.default.join(process.cwd(), "src/app/assets/Logo.png"))
    ? path_1.default.join(process.cwd(), "src/app/assets/Logo.png")
    : path_1.default.join(process.cwd(), "dist/app/assets/Logo.png");
const generatePdf = (invoiceData) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        return new Promise((resolve, reject) => {
            ...
            const fmt = (n) => `BDT ${n.toLocaleString()}`;
            // ── Header bar ──────────────────────────────────────────
            ...
            doc.fontSize(12).fillColor(BRAND_LIGHT).font("Helvetica-Bold")
                .text(`#${invoiceData.transactionId.slice(-8).toUpperCase()}`, 0, 56, { align: "right", width: W - M, lineBreak: false });
            // ── Billed to / Date row ─────────────────────────────────
            let y = 130;
            ...
            doc.fillColor(TEXT_DARK).fontSize(13).font("Helvetica-Bold")
                .text(new Date(invoiceData.bookingDate).toLocaleDateString("en-GB", {
            ...
            doc.fillColor(TEXT_MUTED).fontSize(10).font("Helvetica")
                .text(`${invoiceData.guestCount} guest${invoiceData.guestCount > 1 ? "s" : ""}`, M + 14, y + 32, { lineBreak: false });
            doc.fillColor(TEXT_DARK).fontSize(13).font("Helvetica-Bold")
                .text(fmt(invoiceData.totalAmount), 0, y + 22, { align: "right", width: W - M - 14, lineBreak: false });
            // ── Totals ───────────────────────────────────────────────
            y += 82;
            doc.rect(M, y, W - M * 2, 0.5).fill(BORDER);
            y += 16;
            ...
            doc.fillColor(TEXT_MUTED).fontSize(10).font("Helvetica")
                .text(`Subtotal (${invoiceData.guestCount} x ${fmt(perPerson)})`, M, y);
            doc.fillColor(TEXT_MUTED).fontSize(10)
                .text(fmt(invoiceData.totalAmount), 0, y, { align: "right", width: W - M, lineBreak: false });
            y += 20;
            doc.fillColor(TEXT_MUTED).fontSize(10)
            ...
            doc.fillColor(BRAND_LIGHT).fontSize(8)
                .text(`Transaction ID: ${invoiceData.transactionId}`, M, footerY + 60);
            doc.end();
        });
    }
    catch (error) {
